...
typedef struct snode {
  ...
  uint64_t core;    // core buffer address
  uint32_t len;     // core buffer size
  uint8_t free;
...
static inline int skiplist_insert(skiplist *list, uint64_t lookup,
                                  uint64_t orig, uint64_t core, uint32_t len,
                                  uint64_t offset) {
  snode *update[SKIPLIST_MAX_LEVEL + 1];
  snode *x = list->header;
  int i, level;
  for (i = list->level; i >= 1; i--) {
    while (x->forward[i] && x->forward[i]->lookup < lookup) x = x->forward[i];
    update[i] = x;
  }
  x = x->forward[1];
...
static inline int skiplist_insert_entry(skiplist *list, snode *entry) {
  return skiplist_insert(list, entry->lookup, entry->orig, entry->core,
...
static inline snode *skiplist_search(skiplist *list, uint64_t lookup) {
  ...
  int i;
  for (i = list->level; i >= 1; i--) {
    while (x->forward[i]->lookup < lookup) x = x->forward[i];
  }
...
static inline snode *skiplist_search_buffer_fallin(skiplist *list,
  ...
  snode *x = list->header;
  while (x->forward[1]) {
    if (x->forward[1]->core <= addr &&
        addr < x->forward[1]->core + x->forward[1]->len)
      return x->forward[1];
    x = x->forward[1];
  }
...
static inline void skiplist_dump(skiplist *list) {
  ...
  while (x && x->forward[1] != list->header) {
    printf("%lu[%lu, %lu, %u]->", x->forward[1]->lookup, x->forward[1]->orig,
           x->forward[1]->core, x->forward[1]->len);
    x = x->forward[1];
  }
  printf("NIL\n");
}
